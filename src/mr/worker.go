...
func Worker(mapf func(string, string) []KeyValue,
	...
	for {
		...
		// time.Sleep(time.Second) // 因为有可能在没拿到任务时就执行后面的判断了
		// filename := reply.XTask.FileName
		State := reply.State
		//Response的state状态来判断现在处理的任务
		if CurNumMapTask >= 0 && State == 0 { //map任务
			filename := reply.XTask.FileName
			fmt.Printf(">>>>>> map task!\n")
			...
			CallTaskFin(&reply.XTask) //传参可以知道当前完成了哪些task的id
			// fmt.Printf(">>>>>> send map task finish\n")
			fmt.Printf(">>>>>> map finish !!!!\n")

		} else if CurNumReduceTask >= 0 && State == 1 { //Reduce任务
